...
/**
 * Initialize the GBA.js browser frontend.
 * Call once with a configured GameBoyAdvance instance.
 * Also auto-executes when the script is loaded directly as a page (non-module).
 */
export function main(gba: GameBoyAdvance): void {
  window.gba = gba;
...
// Auto-initialize when loaded as a page script (backward compat)
if (typeof document !== 'undefined') {
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => main(new GameBoyAdvance()));
  } else {
    main(new GameBoyAdvance());
  }
}
